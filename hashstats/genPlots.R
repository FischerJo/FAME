...
gg1 <- gg1 + theme_tufte() + geom_hex(binwidth=c(375000,5))
# gg1 <- gg1 + theme(axis.ticks=element_blank())
gg1 <- gg1 + scale_y_continuous("# hashed values", limits=c(0,400))
gg1 <- gg1 + scale_x_continuous("Bin", breaks=c(0, 10000000, 20000000), labels=c('0', '10mill', '20mill.'))
gg1 <- gg1 + ggtitle("3^20 random 20-mers hashed into 20 Million bins")
gg1 <- gg1 + theme(plot.margin = unit(c(0.5,0,1,1),"cm"))
gg1

gg1_2 <- ggplot(distData, aes(x=Bin, y=Distinct))
gg1_2 <- gg1_2 + theme_tufte() + geom_hex(binwidth=c(375000,5))
# gg1_2 <- gg1_2 + theme(axis.ticks=element_blank())
gg1_2 <- gg1_2 + scale_y_continuous("# hashed values", limits=c(0,400))
gg1_2 <- gg1_2 + scale_x_continuous("Bin", breaks=c(0, 10000000, 20000000), labels=c('0', '10mill', '20mill'))
gg1_2 <- gg1_2 + ggtitle("All distinct 20-mers hashed into 20 Million bins")
...
gg2 <- gg2 + scale_y_log10("# of encounters of so many collisions (logscale)", breaks=c(10,1000,100000),labels=c('10','1thsd','100thsd'))
