...
// ntHash
inline uint64_t NTP64(const char * kmerSeq) {
    uint64_t hVal=0;
    for(unsigned i=0; i<MyConst::KMERLEN; i++)
        hVal ^= msTab[(unsigned char)kmerSeq[i]][(MyConst::KMERLEN-1-i)%64];
    return hVal;
}
...
// canonical ntHash
// CONVENTION: fhVal, rhVal should be initialized to zero by CALLER
inline void NTPC64(const char * kmerSeq, uint64_t& fhVal, uint64_t& rhVal) {
    // fhVal=0, rhVal=0;
    for(unsigned i=0; i<MyConst::KMERLEN; i++) {
        fhVal ^= msTab[(unsigned char)kmerSeq[i]][(MyConst::KMERLEN-1-i)%64];
        rhVal ^= msTab[(unsigned char)kmerSeq[i]&cpOff][i%64];
    }
    // return (rhVal<fhVal)? rhVal : fhVal;
    return;
...
// canonical ntHash for sliding k-mers
inline void NTPC64(const unsigned char charOut, const unsigned char charIn, uint64_t& fhVal, uint64_t& rhVal) {
    fhVal = rol(fhVal, 1) ^ msTab[charOut][MyConst::KMERLEN%64] ^ msTab[charIn][0];
    rhVal = ror(rhVal, 1) ^ msTab[charOut&cpOff][63] ^ msTab[charIn&cpOff][(MyConst::KMERLEN-1)%64];
    // return (rhVal<fhVal)? rhVal : fhVal;
    return;
}
//
