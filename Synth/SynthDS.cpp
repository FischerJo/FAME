...
#pragma omp parallel for num_threads(CORENUM) schedule(dynamic,1000)
        for (size_t i = processedReads; i < ((chr == refSeqFwd.size() - 1) ? readNum : processedReads + chrReadNum); ++i)
        {
            ...
            std::string read2(refSeqFwd[chr], offset + pDist + readLen, readLen);
                        ...
                        switch (read2[j])
                        {
                            ...
                            case ('C') :
                                ...
                                if (j > 0)
                                {
                                    ...
                                    if (read2[j-1] == 'C')
                                    {
                                        struct MethInfo& met = cpgMethRateFwd[ (static_cast<uint64_t>(chr) << 32) | (offset + 2*readLen + pDist - j - 1) ];
                                        std::bernoulli_distribution methIt(met.sampleRate);
                                        if (!methIt(MT))
                                        {
                                            read2[j] = 'A';
                    ...
                    for (size_t j = 0; j < readLen; ++j)
                    {
                        if (read2[j] == 'C')
                        {
                            if (j < readLen - 1)
                            {
                                ...
                                if (read2[j+1] == 'G')
                                {
                                    struct MethInfo& met = cpgMethRateFwd[ (static_cast<uint64_t>(chr) << 32) | (offset + pDist + readLen + j) ];
                                    std::bernoulli_distribution methIt(met.sampleRate);
                                    if (!methIt(MT))
                                    {
                                        read2[j] = 'T';
                        ...
                        switch (read2[j])
                        {
                            case ('G') :
                                ...
                                if (j < readLen - 1)
                                {
                                    if (read2[j + 1] == 'C')
                                    {
                                        struct MethInfo& met = cpgMethRateRev[ (static_cast<uint64_t>(chr) << 32) | (offset + 2*readLen + pDist - j - 2) ];
                                        std::bernoulli_distribution methIt(met.sampleRate);
                                        if (!methIt(MT))
                                        {
                                            read2[j] = 'T';
                    ...
                    for (size_t j = 0; j < readLen; ++j)
                    {
                        if (read2[j] == 'G')
                        {
                            if (j > 0)
                            {
                                ...
                                if (read2[j - 1] == 'C')
                                {
                                    struct MethInfo& met = cpgMethRateRev[ (static_cast<uint64_t>(chr) << 32) | (offset + readLen + pDist + j - 1) ];
                                    std::bernoulli_distribution methIt(met.sampleRate);
                                    if (!methIt(MT))
                                    {
                                        read2[j] = 'A';
                        ...
                        switch (read1[j])
                        {
                            case ('G') :
                                ...
                                if (j < readLen - 1)
                                {
                                    if (read1[j + 1] == 'C')
                                    {
                                        struct MethInfo& met = cpgMethRateRev[ (static_cast<uint64_t>(chr) << 32) | (offset + readLen - j - 2) ];
                                        std::bernoulli_distribution methIt(met.sampleRate);
                                        if (!methIt(MT))
                                        {
                                            read1[j] = 'T';
