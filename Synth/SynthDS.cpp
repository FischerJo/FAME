...
std::vector<std::string> SynthDS::genReadsFwdRef(const size_t readLen, const size_t readNum, const unsigned int maxErrNum, std::vector<std::pair<size_t, size_t> >& offsets)
{
    ...
    for (size_t chr = 0; chr < refSeqFwd.size(); ++chr)
    {
        ...
        const unsigned int chrReadNum = ((double)refSeqFwd[chr].size() / (double)genomeSize) * readNum;
        // TODO
        std::cout << chrReadNum << "\n";
...
#pragma omp parallel for num_threads(CORENUM) schedule(dynamic,1000)
        for (size_t i = processedReads; i < processedReads + chrReadNum; ++i)
        {
...
std::vector<std::string> SynthDS::genReadsRevRef(const size_t readLen, const size_t readNum, const unsigned int maxErrNum, std::vector<std::pair<size_t, size_t> >& offsets)
{
    ...
    for (size_t chr = 0; chr < refSeqRev.size(); ++chr)
    {
        ...
        // compute proportion of reads that should be generated by this chromosome
        const unsigned int chrReadNum = ((double)refSeqRev[chr].size() / (double)genomeSize) * readNum;
        // TODO
        std::cout << chrReadNum << "\n";
